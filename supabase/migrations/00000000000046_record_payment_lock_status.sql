-- 46: record_payment must not hand-roll bill status recompute.
--
-- The inline "select sum(payments) ... update bills set status" blocks in
-- record_payment (introduced m33) read paid_sum WITHOUT locking the bill row,
-- so two concurrent record_payment calls on the same bill can interleave and
-- persist a stale status. refresh_bill_status_for() (m13) already locks the
-- bill row (SELECT ... FOR UPDATE) — delegate to it everywhere. The
-- payments_refresh_bill_status AFTER-INSERT trigger also calls it, so status
-- stays correct even for the leftover/unallocated payment branch.
...
begin
  ...
  -- Oldest-first allocation across all open bills of the customer.
  if v_allocate = 'oldest_first' and v_bill_id is null then
    ...
    loop
      ...
      -- Locking status recompute (SELECT ... FOR UPDATE inside).
  ...
  -- Locking status recompute (replaces unlocked inline sum/update).
