-- Warehouse may create/read bills, but must not see customer balances.
-- Bills SELECT includes warehouse; create_bill allows warehouse (no payment).
-- customer_directory exposes identity fields without financial columns.

-- 1) Bills / bill_items read policies
drop policy if exists "owner sales read bills" on bills;
create policy "staff read bills" on bills
  for select using (
    business_id = current_business_id()
    and current_role_name() in ('owner', 'sales', 'warehouse')
  );

drop policy if exists "owner sales read bill items" on bill_items;
create policy "staff read bill items" on bill_items
  for select using (
    bill_id in (
...
-- Warehouse needs customers SELECT for bill embeds (shop_name only in app queries).
-- Balance stays blocked: no payments SELECT → customer_balances (security_invoker) fails.
drop policy if exists "warehouse reads customers" on customers;
create policy "warehouse reads customers" on customers
  for select using (
    business_id = current_business_id()
    and current_role_name() = 'warehouse'
  );

-- Directory view without financial columns (picker / sync for warehouse).
...
-- 2) create_bill: allow warehouse; reject embedded payment for warehouse
