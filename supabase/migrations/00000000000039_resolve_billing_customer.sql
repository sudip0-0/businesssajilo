-- Offline bills can carry a customer_id from a previous local cache (e.g. after
-- db reset) that no longer exists. Resolve by phone / shop name in the current
-- business before failing create_bill / record_customer_sale.
-- Empty UUID strings ("") used to raise invalid_text_representation.

create or replace function uuid_from_json(v text)
returns uuid
language plpgsql
immutable
...
revoke all on function uuid_from_json(text) from public, anon, authenticated;
  ...
  if v_payment is not null and v_payment != 'null'::jsonb then
    ...
    if v_pay_amount > 0 then
      insert into payments (
        ...
        coalesce(uuid_from_json(v_payment->>'id'), gen_random_uuid()),
        current_business_id(),
        v_customer_id,
        v_id,
        v_pay_amount,
