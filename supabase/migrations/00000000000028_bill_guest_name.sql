-- Optional walk-in display name on bills (not a customers row).

alter table bills
  add column if not exists guest_name text;

comment on column bills.guest_name is
  'Optional display name for walk-in bills; not linked to customers.';

-- Keep guest_name only for walk-ins (null customer_id).
