...
final salesDailyProvider =
    FutureProvider.autoDispose.family<List<SalesPeriodPoint>, ReportRange>(
  (ref, range) {
    final window = dateRangeFor(range);
    return ref.watch(reportsRepositoryProvider).salesDaily(
          from: window.from,
          to: window.to,
        );
  },
...
final topProductsProvider =
    FutureProvider.autoDispose.family<List<TopProductRow>, ReportRange>(
  (ref, range) {
    final window = dateRangeFor(range);
    return ref.watch(reportsRepositoryProvider).topProducts(
          from: window.from,
          to: window.to,
        );
  },
...
final topCustomersProvider =
    FutureProvider.autoDispose.family<List<TopCustomerRow>, ReportRange>(
  (ref, range) {
    final window = dateRangeFor(range);
...
final stockValuationProvider =
    FutureProvider.autoDispose.family<List<StockValuationRow>, bool>(
  (ref, lowStockOnly) {
    return ref
...
final last7DaySalesProvider = FutureProvider.autoDispose<List<SalesPeriodPoint>>(
  (ref) {
